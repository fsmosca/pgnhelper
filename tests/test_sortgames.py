...
def test_sortgames_white_lowtohigh():
    ...
    """
    pgnhelper.sort.sort_games(fn, outfn, 'white', 'lowtohigh')
    white, black = '?', '?'
...
def test_sortgames_white_hightolow():
    ...
    """
    pgnhelper.sort.sort_games(fn, outfn, 'white', 'hightolow')
    white, black = '?', '?'
    with open(outfn, 'r') as f:
        while True:
            game = chess.pgn.read_game(f)
...
def test_sortgames_black_lowtohigh():
    ...
    """
    pgnhelper.sort.sort_games(fn, outfn, 'black', 'lowtohigh')
    white, black = '?', '?'
    with open(outfn, 'r') as f:
        while True:
            game = chess.pgn.read_game(f)
...
def test_sortgames_event():
    ...
    """
    pgnhelper.sort.sort_games(fn, outfn, 'event', 'lowtohigh')
    event = '?',
    with open(outfn, 'r') as f:
        while True:
            game = chess.pgn.read_game(f)
...
def test_sortgames_site(sort_tag='site', sort_direction='lowtohigh', tag_value='Stavanger NOR'):
    ...
    """
    pgnhelper.sort.sort_games(fn, outfn, sort_tag, sort_direction)
    value = '?',
    with open(outfn, 'r') as f:
        while True:
            game = chess.pgn.read_game(f)
